...
piVRHMDOculus::piVRHMDOculus()
{
	...
	mFrame = 0;
	memset( &mTextureChain, 0, sizeof( mTextureChain ) );
...
void piVRHMDOculus::BeginFrame( int *texIndexLeft, int *texIndexRight, bool *outNeedsMipMapping)
{
    ...
    if (ovr_GetInputState(mSession, ovrControllerType_Touch, &inputState) >= 0)
    {
        ...
        if (mInfo.mHead.mController[0].mEnabled )
        {
			...
			iTouchLogic(&mInfo.mHead.mController[0].mThumbstickTouch, (inputState.Buttons & ovrTouch_LThumb) != 0);
			iTouchLogic(&mInfo.mHead.mController[0].mDownTouch, (inputState.Buttons & ovrTouch_X) != 0);
			iTouchLogic(&mInfo.mHead.mController[0].mUpTouch, (inputState.Buttons & ovrTouch_Y) != 0);
			iTouchLogic(&mInfo.mHead.mController[0].mIndexTriggerTouch, (inputState.Buttons & ovrTouch_LIndexTrigger) != 0);
        }
        ...
        if (mInfo.mHead.mController[1].mEnabled)
        {
			...
			iTouchLogic(&mInfo.mHead.mController[1].mThumbstickTouch, (inputState.Buttons & ovrTouch_RThumb) != 0);
			iTouchLogic(&mInfo.mHead.mController[1].mDownTouch, (inputState.Buttons & ovrTouch_A) != 0);
			iTouchLogic(&mInfo.mHead.mController[1].mUpTouch, (inputState.Buttons & ovrTouch_B) != 0);
			iTouchLogic(&mInfo.mHead.mController[1].mIndexTriggerTouch, (inputState.Buttons & ovrTouch_RIndexTrigger) != 0);
        }
